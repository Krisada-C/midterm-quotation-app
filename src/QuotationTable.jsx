...
function QuotationTable({ data, deleteByIndex, clearAll }) {
  ...
  // Calculating totals and formatting to 2 decimal places
  const total = data.reduce((acc, v) => acc + (v.qty * v.ppu - v.discount), 0).toFixed(2);
  const totalDiscount = data.reduce((acc, v) => acc + v.discount, 0).toFixed(2);
  ...
  return (
    <Container>
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {data.map((v, i) => {
            const amount = (v.qty * v.ppu - v.discount).toFixed(2);
            return (
              <tr key={i}>
